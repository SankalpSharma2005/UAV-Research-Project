...
public class TileData : IComparable<TileData>
{
    ...
    public int x, z;
    public float gCost; // Dijkstra only needs gCost
    ...
    // Allows the Priority Queue to sort tiles by gCost
...
/// <summary>
/// Manages grid generation and runs the Dijkstra pathfinding algorithm.
/// </summary>
public class PathManager : MonoBehaviour
{
    ...
    void RunDijkstra()
    {
        ...
        FindPathDijkstra(startTile, endTile);
    }

    // --- Corrected Dijkstra's Algorithm ---
    void FindPathDijkstra(TileData start, TileData end)
    {
        ...
        sw.Stop();
        UnityEngine.Debug.Log("No path found by Dijkstra's.");
    }
    ...
    void TracePath(TileData endTile, double timeMs)
    {
        ...
        UnityEngine.Debug.Log($"Dijkstra path found! Cost: {finalCost:F2}, Time: {timeMs:F3} ms, Memory: {memoryMB:F3} MB");
    ...
    void UpdateLegendText(float cost, double timeMs, bool pathFound, double memoryMB)
    {
        if (legendText != null)
        {
            legendText.text = $"Algorithm: Dijkstra\n" +
